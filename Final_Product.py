...
def be_done():
    ...
    fridge_box= Message(app, textvariable=var_fridge, relief=SUNKEN , width=500).grid(row=11, column= 1)
...
def applyFunction(): #first apply button
...
def check_emo_list(): #checklist emotional list
    Emotions_list = [
...
def check_theme_list(): #checklist theme list
    Theme_list = [
...
def check_lists():
    ...
    can,cant = devide_into_can_cant(search_similar_fridge(dict_fridge,dict_recepies), search_similar_type(theam, dict_type))

    var_can = StringVar()
    var_can_start = ("")
    resualt_can_box = Message(app, textvariable=var_can, relief=SUNKEN , width=300 ).grid(row=11, column= 5)
    ...
    var_cant = StringVar()
    var_cant_start = ("")
    resault_cant_box = Message(app, textvariable=var_cant, relief=SUNKEN , width=300).grid(row=11, column= 7)
...
canvas = app.geometry("1200x350") 
app_name = Label(app, text='Groovy Foody', fg='orange', font = ('Helvetica', 35))
app_name.place(x = 885, y=290)
unit_list = (
    'Single Item(s)',
    'Grams',
     'Kilograms',
...
Label(app, text='Your Fridge:').grid(row=11, column = 0)
Label(app, text='Fridge Result:').grid(row=11, column = 4)
Label(app, text='Possible Results:').grid(row=11, column=6)
...
apply = Button(app, text="Apply", command=applyFunction).grid(column=0)
done= Button( app, text='Done', command=be_done).grid(column= 0)
